class UnitsController < ApplicationController
  ...
  private
    ...
    def top_ids_sales
      @top_ids_sales ||= ProductTicket.joins(:ticket).where('"tickets"."unit_id" = ?', @unit.id)
                                                    .top_by_sales(:product_id, 10)
    end
    def bottom_ids_sales
      @bottom_ids_sales ||= ProductTicket.joins(:ticket).where('"tickets"."unit_id" = ?', @unit.id)
                                                        .bottom_by_sales(:product_id, 10)
    end
